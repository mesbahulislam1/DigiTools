...
const Cart = ({ card, setCard }) => {
  const totalPrice = card.reduce((total, item) => total + item.price, 0).toFixed(2);
  ...
  return (
    <div className="p-7 border border-black/18 rounded-2xl">
...

      {
        ...
        <button onClick={()=> deleteHandel(list)} className="text-[#FF3980] cursor-pointer font-bold">Remove</button>
      </div>
          })
        }
      </div>
      ...
      <div className="space-y-3 mt-4">
        ...
        <button onClick={cartEmpty} className='bg-linear-to-l to-[#4F39F6] w-full from-[#9514FA] px-5 py-2.5 text-white cursor-pointer rounded-full'>Proceed to Checkout</button>
      </div>
        </div>
      }
